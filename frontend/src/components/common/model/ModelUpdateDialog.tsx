...
import type { ModelUpdateReq, PricingItemCreateReq, VendorModelResp, VendorVO, ModelResp } from "@/types/api"
import { toast } from "sonner"

type Props = {
  open: boolean
  onOpenChange: (open: boolean) => void
  model: ModelResp | null
  onUpdated?: () => void
}

export function ModelUpdateDialog({ open, onOpenChange, model, onUpdated }: Props) {
  const [submitting, setSubmitting] = useState(false)
  const [vendorModels, setVendorModels] = useState<VendorModelResp[]>([])
  const [vendors, setVendors] = useState<VendorVO[]>([])
  const [loading, setLoading] = useState(false)

  const [formData, setFormData] = useState<ModelUpdateReq>({
    name: "",
    displayName: "",
    isFree: false,
    category: "chat",
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          setFormData({
            ...
            platformType: "DEEP_SEEK", // 从 model 中可能无法获取，使用默认值
            description: model.description || "",
            vendorModelIds: model.bindings.map((b) => b.venModelId),
            pricingItems:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (modelId === null) {
        toast.error("模型ID无效，无法更新")
        return
      }
      await adminService.updateModel(modelId, formData)
      toast.success("更新成功")
      onOpenChange(false)
      onUpdated?.()
    } catch (error: any) {
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4 overflow-y-auto">
        <div className="w-full max-w-4xl rounded-lg border bg-background shadow-lg max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between border-b px-6 py-4 sticky top-0 bg-background">
            <h2 className="text-lg font-semibold">编辑模型</h2>
